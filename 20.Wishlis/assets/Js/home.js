...
document.addEventListener("DOMContentLoaded", () => {
  ...
  hearts.forEach((heart) => {
    ...
    const isInWishlist = currentUser && currentUser.wishlist.includes(parseInt(productId));
    if (isInWishlist) {
...
products.forEach((product) => {
  ...
  cardTitle.textContent = product.title.split(0, 15) + " ...";

  let heart = document.createElement("i");
  heart.className = "fa-solid fa-heart position-absolute top-0 end-0 m-3 fs-4 text-muted";
  heart.setAttribute('data-product-id', product.id);
  ...
  heart.addEventListener("click", () => {
    ...
    if (currentUser) {
      ...
      const indexInWishlist = currentUser.wishlist.indexOf(productId);
      ...
      if (indexInWishlist === -1) {
        ...
        currentUser.wishlist.splice(indexInWishlist, 1);
